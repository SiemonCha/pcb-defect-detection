...
PRODUCTION_CFG = CONFIG.get("training", {}).get("production", {})
...
def train_production(
    *,
    epochs: int = PRODUCTION_CFG.get("epochs", 100),
    imgsz: int = PRODUCTION_CFG.get("imgsz", 640),
    batch: int = PRODUCTION_CFG.get("batch", 16),
    patience: int = PRODUCTION_CFG.get("patience", 15),
    model_path: str = "yolov8s.pt",
    project: str = "runs/train",
    name: str = "production_yolov8s",
):
    """Train the YOLOv8s production-tuned model."""
    ...
    if tracking_enabled:
        ...
        run_name = f"{prefix}-production-{timestamp}"
...
def main(args: Optional[Iterable[str]] = None):
    ...
    parser.add_argument("--epochs", type=int, default=PRODUCTION_CFG.get("epochs", 100))
    parser.add_argument("--imgsz", type=int, default=PRODUCTION_CFG.get("imgsz", 640))
    parser.add_argument("--batch", type=int, default=None, help="Override batch size")
    parser.add_argument("--patience", type=int, default=PRODUCTION_CFG.get("patience", 15))
