...
def _load_module(module_path: str):
    try:
        ...
        print(
            ...
            missing,
            "'. Install the prerequisites listed in requirements.txt or disable the command requiring it.",
            sep="",
        )
        return None
...
def cmd_benchmark_onnx(args: argparse.Namespace) -> int:
    cli_args = [str(args.model)]
    if args.runs is not None:
        cli_args += ["--runs", str(args.runs)]
    if args.warmup is not None:
        cli_args += ["--warmup", str(args.warmup)]
    if args.imgsz is not None:
        cli_args += ["--imgsz", str(args.imgsz)]
    if args.providers:
        cli_args += ["--providers", *args.providers]
    benchmark_module = _load_module("benchmarks.onnx")
    if not benchmark_module:
        return 1

    return _run_cli(benchmark_module.main, cli_args)
...
def _register_commands(subparsers: argparse._SubParsersAction) -> None:
    def add_command(name: str, handler, help_text: str, description: str | None = None) -> argparse.ArgumentParser:
    ...
    add_command(
        "dataset-analysis",
        cmd_dataset_analysis,
        "Analyze dataset statistics for imbalance and annotation gaps",
        "Run summary statistics over the configured dataset, reporting class distribution, bbox counts, and imbalance metrics.",
    )

    parser_fail = add_command(
        "failure-analysis",
        cmd_failure_analysis,
        "Visualize mispredictions and matching ground truth",
        "Render side-by-side comparison plots highlighting false positives, false negatives, and low-IoU detections.",
    )
    ...
    parser_interp = add_command(
        "interpretability",
        cmd_interpretability,
        "Generate confidence overlays for predictions",
        "Produce attention heatmaps and annotated predictions for a sample of dataset images.",
    )
    ...
    parser_quick = add_command(
        "quick-analysis",
        cmd_quick_analysis,
        "Execute the curated analysis bundle",
        "Runs dataset analysis, interpretability, and optional robustness/failure case inspections in sequence.",
    )
    ...
    parser_robust = add_command(
        "robustness",
        cmd_robustness,
        "Benchmark robustness against corruptions",
        "Applies noise/blur/occlusion corruptions to a subset of images and compares detections with baseline results.",
    )
    ...
    parser_check_ds = add_command(
        "check-dataset",
        cmd_check_dataset,
        "Validate dataset annotations and segmentation",
        "Reports missing labels, segmentation masks, and other dataset issues (optionally auto-fixes segmentation).",
    )
    parser_check_ds.add_argument("--auto-fix", action="store_true", help="Convert COCO segmentation to YOLO boxes")

    parser_check_img = add_command(
        "check-images",
        cmd_check_images,
        "Validate image value ranges",
        "Detects non-uint8 image data and optionally converts to standard range.",
    )
    ...
    add_command(
        "verify-setup",
        cmd_verify_setup,
        "Verify Python dependencies and accelerator support",
        "Checks that required packages and optional accelerators (CUDA/ROCm/MPS) are available.",
    )
    add_command(
        "data-download",
        cmd_data_download,
        "Download dataset using configured provider",
        "Invokes the dataset downloader with credentials defined in configuration overrides.",
    )

    parser_train_base = add_command(
        "train-baseline",
        cmd_train_baseline,
        "Train the lightweight YOLOv8 baseline",
        "Runs the baseline training loop with sensible defaults for experimentation.",
    )
    ...
    parser_train_prod = add_command(
        "train-production",
        cmd_train_production,
        "Train the production-tuned YOLOv8 model",
        "Runs the high-capacity YOLOv8 configuration with extended patience and batch sizes.",
    )
    ...
    parser_transfer = add_command(
        "transfer-learning",
        cmd_transfer_learning,
        "Fine-tune pretrained weights on a new dataset",
        "Provides a thin wrapper around the transfer-learning script for quick adaptation to custom datasets.",
    )
    ...
    parser_eval = add_command(
        "evaluate",
        cmd_evaluate,
        "Evaluate a trained model on the test split",
        "Generates evaluation metrics, summary reports, and JSON logs for the configured dataset.",
    )
    parser_eval.add_argument("model", nargs="?", help="Model weights path (auto-detect if omitted)")

    parser_conf = add_command(
        "confusion",
        cmd_confusion,
        "Generate confusion matrices and per-class summaries",
        "Validates a model and renders row-normalised confusion matrices alongside per-class AP metrics.",
    )
    ...
    parser_export = add_command(
        "export-onnx",
        cmd_export_onnx,
        "Export YOLO checkpoints to ONNX",
        "Converts a trained checkpoint to ONNX (with optimisations) ready for deployment.",
    )
    parser_export.add_argument("model", nargs="?", help="Model weights path (auto-detect if omitted)")

    parser_quant = add_command(
        "quantize",
        cmd_quantize,
        "Run INT8 quantisation of ONNX models",
        "Benchmarks, exports, and optionally quantises ONNX representations for deployment.",
    )
    ...
    parser_monitor = add_command(
        "monitor",
        cmd_monitor,
        "Probe inference API latency and accuracy",
        "Hits the API on an interval, logging latency, accuracy, and error rates to disk.",
    )
    ...
    parser_api = add_command(
        "api",
        cmd_api,
        "Launch the FastAPI inference service",
        "Serves YOLO predictions over HTTP using FastAPI.",
    )
    ...
    parser_bench = add_command(
        "benchmark-onnx",
        cmd_benchmark_onnx,
        "Benchmark ONNX inference latency",
        "Runs random input through an ONNX model to measure latency/throughput across providers.",
    )
    parser_bench.add_argument("model", type=str, help="Path to ONNX model")
    parser_bench.add_argument("--runs", type=int, default=100, help="Timed iterations")
    parser_bench.add_argument("--warmup", type=int, default=10, help="Warmup iterations")
    parser_bench.add_argument("--imgsz", type=int, default=640, help="Input image size")
    parser_bench.add_argument("--providers", nargs="*", help="Execution providers list")


def main(argv: Optional[Iterable[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="PCB Defect Detection CLI",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        epilog="See README.md for end-to-end workflows and examples.",
    )
    ...
    if handler is None:
        print("[ERROR] Unknown command. Run 'pcb-dd --help' to list available commands.")
        return 1
    return handler(args)
